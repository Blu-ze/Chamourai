...
\textit{Chamouraï} est un jeu d'action-aventure en vue du dessus, réalisé en Python avec
...
ennemis et une carte. La version actuelle comprend un tutoriel intégré, des objectifs
successifs, plusieurs familles d'ennemis, des boss, un verrouillage du donjon par
clés, une carte consultable, des musiques contextuelles et un mode coopératif avec
gestion de la mort et de la reprise de partie.

Le jeu conserve une inspiration de type \textit{die and retry} : perdre fait partie de
l'apprentissage. Nous avons cependant cherché à rendre les échecs compréhensibles.
Les animations annoncent les attaques, la barre de vie du boss donne un repère clair
...
\hline
Lucas HUET & Interfaces, menus, musiques, effets sonores et réglages audio \\
\hline
Loan BLANPAIN & Site web, scénario, univers et communication du projet \\
\hline
...
donc été l'intégration : tester ensemble les ajouts de chacun et résoudre les conflits
...
validés. Ce choix évite qu'un joueur se retrouve face aux boss sans avoir utilisé les
actions essentielles. En multijoueur, les éliminations utiles au tutoriel sont
...
\imageplaceholder{Tutoriel et objectifs successifs}{Capture du début de partie avec le panneau d'objectif affichant, par exemple, « Frapper 3 fois » et sa progression.}
...
La grille qui ferme l'accès au golem constitue l'objectif central du donjon. Deux boss
...
interaction relie combat et exploration : éliminer les boss a une conséquence visible
sur la carte.

\imageplaceholder{Carte du niveau et position du joueur}{Capture de la carte ouverte avec la position du joueur, les principales salles et la zone du boss final.}
...
Cette scène est simple, mais elle donne une présence au monde et permet de transmettre
une indication narrative sans créer un écran de dialogue complexe. Nous avons ralenti
l'animation de la touche d'interaction afin qu'elle attire l'attention sans clignoter
de manière gênante.
...
bloquante toute tuile non vide du calque \texttt{Walls}. Le level designer peut ainsi
dessiner un mur et obtenir immédiatement une collision cohérente.

L'accès au boss final a nécessité une distinction importante. Le calque de tuiles
...
\subsection{Mode Invincible}

Une combinaison \texttt{CTRL+A} active un mode de test nommé \textit{Invincible}. Il
augmente fortement la vitesse et les dégâts du joueur et empêche les dégâts reçus.
Ce mode est utile pour vérifier rapidement une salle, l'ouverture de la grille ou les
phases du golem sans rejouer l'intégralité du niveau. Il reste identifié à l'écran et
peut être désactivé avec la même combinaison, afin de ne pas le confondre avec
l'équilibrage normal du jeu.

\sectionbreak
\section{Ennemis et boss}
...
plutôt qu'à attendre derrière son épée. Chacun existe également en version boss, avec
un sprite agrandi, une barre de vie visible au-dessus de sa tête, davantage de vie,
...
\subsection{Boss intermédiaires et clés}
...
qui préparent au boss final. À leur mort, ils déposent chacun une clé représentée par
...
\imageplaceholder{Boss intermédiaire et clé déposée}{Capture d'un boss avec sa barre de vie, puis de la clé apparue au sol après sa défaite.}

\subsection{Le golem, boss final}
...
Nous avions envisagé un rayon laser continu sortant de sa tête. Sa gestion s'est
...
frames. Nous avons remplacé cette idée par une onde de choc. Elle conserve le rôle
...
Le jeu adopte un style pixel art en vue du dessus. Le personnage principal a été
...
L'ajout du vieil homme, des clés et des différents boss a demandé une attention
particulière aux dimensions de sprites : un boss doit sembler plus massif tout en
conservant des collisions jouables. Le golem, notamment, utilise des images plus
grandes que les autres personnages et des sprites séparés pour ses projectiles.

\subsection{Menus et informations en jeu}

Le menu principal permet d'accéder au jeu solo, au multijoueur, aux options et à la
sortie. Le menu multijoueur offre la création ou la connexion à une partie et comprend
...
\imageplaceholder{Menu multijoueur}{Capture du menu permettant de rejoindre ou créer une partie, avec le bouton Retour visible.}
...
musique de boss prend le relais lorsque la grille s'ouvre. Le menu des sons permet de
régler l'expérience sans modifier le code.

L'intégration audio a posé un problème fréquent dans un projet en équipe : la boucle
principale était également modifiée par les objectifs, les boss et le multijoueur.
...
\sectionbreak
\section{Mode multijoueur coopératif}

\subsection{Principe de connexion}

Le multijoueur est conçu pour deux joueurs. L'un héberge la partie et l'autre rejoint
...
permanente tout en offrant une expérience coopérative. Il demande cependant de ne pas
faire confiance uniquement à l'affichage local. Si chaque ordinateur déplaçait ses
propres monstres, un boss pourrait être mort sur un écran et vivant sur l'autre.
...
boss. Le doublement des points de vie est un réglage simple, cohérent et compréhensible
pour maintenir l'intérêt des combats coopératifs.

\subsection{Mort, observation et reprise}

Lorsqu'un joueur meurt en multijoueur, il disparaît du combat et peut observer la
...
imposer à nouveau le tutoriel.

\imageplaceholder{Partie coopérative et joueur observateur}{Capture multijoueur où un joueur combat tandis que l'autre, mort, observe la partie avec sa caméra centrée sur son partenaire.}

\imageplaceholder{Défaite et vote de reprise}{Capture de l'écran de défaite multijoueur présentant le bouton de retour et le bouton de vote pour recommencer.}
...
calque ont transformé une tâche manuelle et fragile en une opération de level design.

\subsection{Concilier lisibilité et ambition des boss}

Les attaques spectaculaires sont intéressantes seulement si le joueur comprend comment
y répondre. Le laser du golem était ambitieux, mais son rendu et sa collision
...
L'arrivée du multijoueur a révélé que de nombreux systèmes étaient liés : tuer un boss
...
l'hôte les décisions importantes et transmis les états nécessaires aux clients.

Les reprises de partie ont accentué cette difficulté. Relancer uniquement les joueurs
laisserait une grille déjà ouverte ou un boss déjà vaincu. Le redémarrage doit remettre
en cohérence tout l'état du donjon. Ce travail est moins visible qu'un sprite ou un
menu, mais il conditionne la fiabilité de l'expérience coopérative.

\subsection{Intégrer le travail de plusieurs branches}

Le développement en parallèle a permis d'avancer rapidement sur l'audio, les ennemis,
le tutoriel et le réseau. En contrepartie, certaines fusions ont créé des conflits
...
les boutons de reprise et les nouvelles règles multijoueur.
...
audio dans l'interface, synchronisation dans le serveur. Plus chaque fonction possède
un emplacement clair, plus une future évolution est testable et intégrable.
...
réunions vocales. Il permet de signaler rapidement un bug, de montrer une capture ou
...
intégrer et celles à tester. Les réunions hebdomadaires du jeudi matin servent à
...
Dans les dernières phases, l'organisation des tests est devenue aussi importante que
l'ajout de fonctionnalités : une mécanique correcte en solo doit être revue en
coopératif, après une mort, après une reprise et après une transition de carte.
...
d'ennemis variés, récupère les clés auprès de boss intermédiaires et termine sur un
...
synchronisation multijoueur permet à la progression de rester cohérente. Enfin, les
choix effectués pour le golem et le tutoriel montrent notre priorité : proposer une
expérience claire, équilibrée et agréable avant de multiplier les effets difficiles à
maîtriser.

Pour une version future, nous pourrions compléter les tests réseau sur des connexions
moins stables, enrichir le contenu du parchemin et ajouter de nouvelles salles ou
variantes d'ennemis grâce au système de placement dans Tiled déjà en place.
...
\imageplaceholder{Liste des ressources visuelles complémentaires}{Emplacement éventuel pour une planche réunissant les sprites des clés, du vieil homme, des boss et du golem.}
