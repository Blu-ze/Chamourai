...
\newcommand{\singleimage}[3][0.76\textwidth]{%
    \begin{figure}[H]
        \centering
        \includegraphics[width=#1, height=0.53\textheight, keepaspectratio]{#2}
        \caption{#3}
    \end{figure}
}
\newcommand{\doubleimage}[3]{%
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.46\textwidth, height=0.34\textheight, keepaspectratio]{#1}
        \hfill
        \includegraphics[width=0.46\textwidth, height=0.34\textheight, keepaspectratio]{#2}
        \caption{#3}
    \end{figure}
}
...
\title{\textbf{\textit{Chamouraï}} \\ Rapport de soutenance}
\author{\textit{Les Chatpitres} \\[0.35cm]
\textit{Baptiste LOISEAU} \quad \textit{Lucas HUET} \\
\textit{Loan BLANPAIN} \quad \textit{Simon BOURGEOIS}}
\date{27/05/2026}
...
\textit{Chamouraï} est un jeu d'action et d'aventure en vue du dessus, réalisé en \textit{Python} avec
\textit{Pygame}. Le joueur incarne un chat samouraï qui doit progresser d'une forêt vers un
...
\singleimage{image1.png}{Vue générale d'une partie de \textit{Chamouraï}}
...
\hline
\textit{Baptiste LOISEAU} & Chef de projet, conception des cartes, jouabilité, intégration graphique et équilibrage des combats \\
\hline
\textit{Lucas HUET} & Interfaces, menus, musiques, effets sonores et réglages du son \\
\hline
\textit{Loan BLANPAIN} & Site internet, scénario, univers et communication du projet \\
\hline
\textit{Simon BOURGEOIS} & Intelligence artificielle, déplacements des ennemis et multijoueur \\
\hline
\end{tabular}
\end{center}
...
zone créée dans \textit{Tiled} influence les collisions, l'intelligence artificielle, les
...
\begin{enumerate}
    ...
    \item effectuer un coup chargé afin de découvrir une attaque plus puissante ;
...
\singleimage[0.43\textwidth]{image2.png}{Premier objectif de l'apprentissage guidé}
...
\singleimage{image3.png}{Carte du niveau et position du joueur}

\singleimage{image4.png}{Grille fermée et emplacements de clés}
...
\doubleimage{image5-1.png}{image5-2.png}{Interaction avec le vieil homme et lecture du parchemin}

\sectionbreak
\section{Conception technique des cartes}

\subsection{Cartes produites avec \textit{Tiled}}

Les niveaux sont dessinés avec \textit{Tiled}, qui permet de superposer des calques de tuiles et
...
positions directement en \textit{Python} : la carte peut être retouchée visuellement sans
...
l'intérieur minéral du donjon. Le travail par calques dans \textit{Tiled} nous a permis
d'ajouter du relief visuel et des éléments de décor sans perdre la lisibilité des
chemins et des combats.

Pour les ennemis, chaque objet \textit{Tiled} portant le nom \texttt{Skeleton},
...
\singleimage{image6.png}{Édition du niveau dans \textit{Tiled}}
...
objet \textit{Tiled} fournit un rectangle. Après ouverture, le calque visuel de la grille est
...
\doubleimage{image7-1.png}{image7-2.png}{Attaque orientée par le curseur dans deux directions}

\subsection{Coup chargé et prise de risque}

Nous avons enrichi l'épée avec une attaque chargée. En maintenant le bouton de la
souris, le joueur prépare son coup ; une barre de charge apparaît alors à l'écran et
indique clairement quand l'attaque a atteint sa pleine puissance. Au relâchement, le
coup utilise une animation différente de celle d'une attaque normale, afin que les
deux actions soient immédiatement reconnaissables, et inflige quatre fois les dégâts
d'une attaque normale.

Cette mécanique apporte un choix supplémentaire au combat. Frapper immédiatement
permet de rester réactif face à un ennemi proche, tandis que charger l'épée permet de
réduire fortement la vie d'un adversaire résistant. En contrepartie, attendre avant
de frapper expose davantage le joueur aux attaques. Nous avons placé cette découverte
dans l'apprentissage guidé, juste après les trois frappes normales, afin que le joueur
comprenne la différence entre rapidité et puissance avant ses premiers affrontements.

\doubleimage{image15.png}{image16.png}{Barre de préparation et animation du coup chargé, quatre fois plus puissant}
...
Les monstres utilisent une recherche de chemin basée sur \textit{A*}. Nous avons préféré \textit{A*} à
\textit{Dijkstra} car notre objectif n'est pas d'explorer toutes les distances possibles, mais
...
L'intégration a été délicate : un ennemi doit éviter les murs de \textit{Tiled}, garder une
...
\singleimage[0.55\textwidth]{image8.png}{Affrontement avec différents ennemis}
...
\doubleimage{image9-1.png}{image9-2.png}{Adversaire intermédiaire et clé déposée}
...
\singleimage{image10.png}{Combat contre le golem en seconde phase}

\singleimage{image11.png}{Écran de victoire après la défaite du golem}
...
\singleimage[0.48\textwidth]{image12.png}{Menu du jeu à deux}
...
musique de combat final prend le relais lorsque la grille s'ouvre. Le menu des sons permet au
joueur d'ajuster librement le volume, avec un réglage indépendant pour la musique et pour les
effets sonores. Il peut ainsi conserver l'ambiance musicale tout en atténuant les bruits
d'action, ou au contraire mieux entendre les indications liées au combat.
...
\newpage
\subsection{Installation et désinstallation}

Pour rendre \textit{Chamouraï} facilement accessible, nous avons préparé un programme
d'installation au format exécutable. Son utilisation est volontairement simple : le
joueur le lance, choisit l'emplacement où il souhaite installer le jeu, puis peut
accéder à celui-ci sans avoir à configurer manuellement son environnement. Un
programme de désinstallation est également fourni afin de retirer proprement le jeu
de l'ordinateur lorsque l'utilisateur ne souhaite plus le conserver.

Lors de l'exécution du programme d'installation, \textit{Windows} peut néanmoins
afficher un avertissement de sécurité. Cet avertissement ne provient pas d'un problème
identifié dans le jeu, mais du fait que notre exécutable n'est pas signé par un
certificat reconnu par \textit{Microsoft}. En tant qu'équipe étudiante, nous ne sommes
pas enregistrés comme éditeur de confiance auprès du système. Pour une diffusion plus
large, une évolution importante consisterait donc à signer numériquement le programme
d'installation, afin d'identifier clairement son origine et de rassurer davantage les
utilisateurs au moment de son lancement.
...
pour maintenir l'intérêt des combats en coopération. L'équilibrage ne consiste cependant
pas à rendre les deux modes identiques : en partie seule, la vie du golem est également
renforcée afin que l'affrontement final reste exigeant malgré la concentration des
actions sur un seul joueur.
...
\singleimage{image13.png}{Partie en coopération avec un joueur observateur}

\singleimage{image14.png}{Défaite à deux joueurs et vote de reprise}
...
frein dès que la carte changeait. L'utilisation des objets \textit{Tiled} a demandé de définir
...
\subsection{Obtenir un équilibrage jouable par les essais}

L'équilibrage n'a pas pu être défini correctement dès la première version. Une valeur
qui semblait convenir face à un squelette isolé devenait trop avantageuse lorsque
plusieurs ennemis se superposaient, et un combat satisfaisant seul pouvait devenir trop
court ou au contraire trop difficile à deux joueurs. Nous avons donc dû essayer
régulièrement les combats en situation réelle : dans la forêt, devant les adversaires
intermédiaires, puis face au golem dans ses deux phases.

Ces essais ont conduit à plusieurs ajustements. Les squelettes ont reçu un espacement
pour ne plus former un groupe facile à toucher d'un seul coup ; les nécromanciens ont
été rendus moins résistants afin que leurs attaques à distance restent menaçantes sans
rendre les affrontements trop longs ; les adversaires intermédiaires conservent une
vie nettement supérieure pour marquer leur rôle dans la progression. Le golem a aussi
été ajusté séparément, car sa durée de combat, ses attaques et son changement de phase
déterminent largement le rythme de la fin du niveau.

L'ajout du coup chargé a nécessité une nouvelle série d'essais. Comme cette attaque
récompense le joueur par des dégâts élevés, il fallait vérifier qu'elle ne supprimait
pas l'intérêt de l'esquive ni la dangerosité des adversaires majeurs. Nous avons
également vérifié qu'un coup ne pouvait pas retirer plusieurs fois de la vie au même
ennemi au cours d'une seule animation, et que les animations de blessure restaient
compréhensibles en coopération.

Enfin, le jeu à deux a demandé des vérifications particulières : synchronisation des
points de vie, élimination d'un ennemi visible sur les deux écrans, récupération des
clés, ouverture de la grille, mort d'un joueur, observation de la partie et reprise
après une défaite. Cette succession d'essais et de corrections a permis d'obtenir une
version jouable, dans laquelle la difficulté progresse sans rendre les règles
incompréhensibles ou injustes.
...
de demander une vérification après une intégration. \textit{Instagram} est utilisé de manière
complémentaire pour des messages rapides et les imprévus.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.58\textwidth]{discord.png}
    \caption{Serveur \textit{Discord} du groupe \textit{Les Chatpitres}}
\end{figure}

\subsection{Gestion des versions}

Nous utilisons \textit{Git} pour répartir le développement et conserver l'historique du
projet. Afin de réduire les conflits, nous communiquons sur \textit{Discord} avant d'intervenir
sur une même branche ou sur un même fichier important. Les images et les sons étant
volumineux, nous utilisons également \textit{Git LFS}, une extension de \textit{Git} adaptée au suivi
de ces fichiers.
...
\begin{figure}[H]
    ...
    \caption{Tableau de suivi des tâches sur \textit{MeisterTask}}
\end{figure}
...
étaient difficiles à exploiter correctement avec \textit{Pygame} ; l'onde de choc en arc de
...
\begin{figure}[H]
    ...
    \caption{Groupe \textit{Instagram} de communication rapide}
\end{figure}

\singleimage{annexe1.png}{La carte de départ dans son entièreté}

\singleimage[0.45\textwidth]{annexe2.png}{L'adversaire intermédiaire nécromancien}
